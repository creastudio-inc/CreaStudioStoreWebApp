...
/**
 * CreaStore Documentation Javascript File
 */
"use strict";
...
( function () {
    ...
    var Docs = {
        init: function () {
            ...
            // # Register events
            CreaStore.$body.on( 'click', '.document-link', function ( e ) {
                self.open( e.currentTarget.getAttribute( 'href' ).slice( 1 ) );
                var mainContent = CreaStore.byClass( 'main-content' );
        ...
        open: function ( id ) {
            ...
            if ( activeId && templates[activeId] ) {
                CreaStore.byId( 'document-view' ).innerHTML = templates[activeId].text;
                CreaStore.byId( 'document-title' ).textContent = templates[activeId].title;
                CreaStore.byId( 'document-view' ).classList.remove( 'search-result' );
            }
            $( CreaStore.byClass( 'document-link' ) ).parent( 'li' ).removeClass( 'show' );
        ...
        search: function ( query ) {
            ...
            CreaStore.byId( 'document-title' ).textContent = 'Search Result';
            ...
            CreaStore.byId( 'document-view' ).innerHTML = finalResult ? finalResult : '<h5 class="text-center">Nothing Found</h5>';
            CreaStore.byId( 'document-view' ).classList.add( 'search-result' );
        }
    }

    CreaStore.docs = Docs;
    $( window ).on( 'wolmart_complete', function () {
        CreaStore.docs && CreaStore.docs.init();
    } )
} )();